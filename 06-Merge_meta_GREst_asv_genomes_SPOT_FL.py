...
def generate_df(ii):
    ...
    sample = str(year) + '_' + str(month) + '_1_' + sample1 + '_' + 'FL'
            ...
            if genomes.__len__() > 0:
                for genome in genomes.values:
                    ...
                    if matched_genome.__len__() > 0:
                        ...
                        pred_d_FL_genome = pred_d[pred_d['genomeid'] == genome]['d_FL'].values[0]
                        OGT_FL = pred_d[pred_d['genomeid'] == genome]['OGT_FL'].values[0]
...
# save the full data
full_data.to_csv('data/SPOT_GRest_meta_FL.csv', index=False)
