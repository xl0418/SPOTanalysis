...
def generate_df(ii):
    ...
    pred_g = pd.read_csv('data/SPOT_EstG_genome_full.tsv', sep='\t')
        ...
        for pres_tag in present_tags.index:
            ...
            if genomes.__len__() > 0:
                for genome in genomes.values:
                    matched_genome = pred_g[pred_g['genomeid']==genome]
                    if matched_genome.__len__() > 0:
                        pred_g_genome = pred_g[pred_g['genomeid']==genome]['d'].values[0]
                        lowerci = pred_g[pred_g['genomeid']==genome]['LowerCI'].values[0]
                        upperci = pred_g[pred_g['genomeid']==genome]['UpperCI'].values[0]
                        dCUB = pred_g[pred_g['genomeid']==genome]['dCUB'].values[0]
                        nHE = pred_g[pred_g['genomeid']==genome]['nHE'].values[0]
                        ...
                        # if want to get estimates for all genomes, remove the break
                        break
    return batch_data
...
# save the full data
full_data.to_csv('data/SPOT_1hits_FL.csv', index=False)