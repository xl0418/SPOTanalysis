...
# read  the data
p16ns_d_est = pd.read_csv("data/SPOT_allhits_meta_FL.csv", sep=',')
...
def get_d_est_for_asvs(tag):
    ...
    mean_d_tag_vT = tag_df['d_vT'].mean()
    # unique tag_df by depth, sample, and Lat
    unique_tag_df = tag_df.drop_duplicates(subset=['depth', 'sample', 'Lat'])
    unique_tag_df = unique_tag_df.loc[:, ['depth', 'sample', 'Lat', 'relative_a', 'temp', 'tag']]
...
if __name__ == '__main__':
    ...
    mean_d_tag_df.to_csv('data/SPOT_mean_d_meta_FL.csv', index=False)
